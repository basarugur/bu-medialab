...
void TriangleMesh::copyToMesh(TriangleMesh* msh_)
{
	...
	for (int i=0 ; i<m_facelist.size() ; i++ )
	{
		...
		std::ostringstream tmpKey_;
		tmpKey_ << v1_->x() << ";" << v1_->y() << ";" << v1_->z() ;
		str_ = tmpKey_.str();
		if (vertexHash_.find(str_) != vertexHash_.end() )
		{
			v1_ = msh_->vertexList()[vertexHash_[str_]];
		}
		else
		{
			msh_->vertexList().push_back(new Vertex(v1_->x(),v1_->y(),v1_->z()));
			v1_ = msh_->vertexList()[msh_->vertexList().size()-1];
			vertexHash_.insert(std::pair<std::string,int>(str_,msh_->vertexList().size()-1));
		}

		// v2
		std::ostringstream tmpKey1_;
		tmpKey1_ << v2_->x() << ";" << v2_->y() << ";" << v2_->z() ;
		str_ = tmpKey1_.str();
		if (vertexHash_.find(str_) != vertexHash_.end() )
		{
			v2_ = msh_->vertexList()[vertexHash_[str_]];
		}
		else
		{
		...
		// v3
		std::ostringstream tmpKey2_;
		tmpKey2_ << v3_->x() << ";" << v3_->y() << ";" << v3_->z() ;
		str_ = tmpKey2_.str();
		if (vertexHash_.find(str_) != vertexHash_.end() )
		{
			v3_ = msh_->vertexList()[vertexHash_[str_]];
		}
		else
		{
