...
class SM_API_EXPORT GfxObject : public Primitive
{
	public:
		...
		{
			m_public_transform = new Transformation();
			*m_public_transform = *t;
		...
		~GfxObject()
		{
		    delete m_public_transform;
		    delete m_individual_transform;
		};
		...
		Transformation* getPublicTransform(){ return m_public_transform; }
		void setPublicTransform(Transformation* pbl_){ *m_public_transform = *pbl_; }
		Transformation* getIndividualTransform(){ return m_individual_transform; }
	...
	private:
		...
		Transformation* m_public_transform;
		GfxObject* m_parent;
