...
void VrmlDevice::saveMeshToIndexedSet(TriangleMesh* msh_ , IndexedFaceSetNode* fst_)
{
	...
	for (int k=0 ; k< msh_->vertexList().size() ; k++)
	{
		...
		vertexHash_.insert(QString::number(vertexf[0],'f',3)
			              +QString::number(vertexf[1],'f',3)
						  +QString::number(vertexf[2],'f',3),k);
	}

	for (int k=0 ; k< msh_->faceList().size() ; k++)
	{
		fst_->getCoordIndexField()->addValue(vertexHash_[QString::number(msh_->faceList()[k]->a()->x(),'f',3)
														+QString::number(msh_->faceList()[k]->a()->y(),'f',3)
														+QString::number(msh_->faceList()[k]->a()->z(),'f',3)] );
		fst_->getCoordIndexField()->addValue(vertexHash_[QString::number(msh_->faceList()[k]->b()->x(),'f',3)
														+QString::number(msh_->faceList()[k]->b()->y(),'f',3)
														+QString::number(msh_->faceList()[k]->b()->z(),'f',3)] );
		fst_->getCoordIndexField()->addValue(vertexHash_[QString::number(msh_->faceList()[k]->c()->x(),'f',3)
														+QString::number(msh_->faceList()[k]->c()->y(),'f',3)
														+QString::number(msh_->faceList()[k]->c()->z(),'f',3)] );
