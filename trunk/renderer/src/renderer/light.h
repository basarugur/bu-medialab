	...
	class Light
	{
		public:	
			...
			virtual output::TRadiance s(const Ray& v,DifferentialGeometry* dg) const {			
				...
				r = 0.3*dg->material()->ambientcolor().r*dg->material()->diffcolor().r;
				g = 0.3*dg->material()->ambientcolor().g*dg->material()->diffcolor().g;
				b = 0.3*dg->material()->ambientcolor().b*dg->material()->diffcolor().b;				
	...
	class PointLight : public Light
	{
		public:
			...
			virtual output::TRadiance l(const Ray& v,DifferentialGeometry* dg) const {
				...
				r = 0.3*dg->material()->ambientcolor().r*dg->material()->diffcolor().r + 1.0*dg->material()->diffcolor().r*shade + 0.9*dg->material()->speccolor().r*spec;
				g = 0.3*dg->material()->ambientcolor().g*dg->material()->diffcolor().g + 1.0*dg->material()->diffcolor().g*shade + 0.9*dg->material()->speccolor().g*spec;
				b = 0.3*dg->material()->ambientcolor().b*dg->material()->diffcolor().b + 1.0*dg->material()->diffcolor().b*shade + 0.9*dg->material()->speccolor().b*spec;					
