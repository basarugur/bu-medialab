...
void Scene::render()
{
	const SMScreen* scr = m_sampler->screen();
	...
	CImgDisplay disp(*image,"MediaLab VW 2009 - Bogazici University");
	...
	if(m_aa_on == true) {
		...
		SMScreen* scr_aa = new SMScreen(scr->width(),scr->height());
	...
	image->blur(0.5);
	image->draw_text("MediaLab VW 2009 - Bogazici University",1,1,white,black,8);
	disp.display(*image);
...
void Scene::anti_alias(SMScreen* s,int x, int y) {
