...
struct FaceLessFunctor
{
	...
	bool operator()(const Triangle* f0, const Triangle* f1) const
	{
...
struct BoxLessFunctor
{
	...
	bool operator()(const KdtreeNode* f0, const KdtreeNode* f1) const
	{
	...
	bool Kdtree::intersect(const Ray& r,Intersection* isect) const {
		return examine(r,isect);
	}
	bool Kdtree::intersectp(const Ray& r) const {
		return examine_shadow(r);
	}

	void Kdtree::build(int maxdepth,int threshold,std::vector<Triangle*>& list)
	{		
		delete m_root;
		m_root = build_recursion(list,0);		
	}

	void Kdtree::split(const std::vector<Triangle*>& list,std::vector<Triangle*>& leftlist,std::vector<Triangle*>& rightlist,double median,int axis)
	{
	...
	KdtreeNode* Kdtree::build_recursion(std::vector<Triangle*> list,int depth)
	{
		if (depth < 21)
		{
			if(list.size() == 0)
	...
	//for primary ray
	void Kdtree::intersect_nodes(KdtreeNode* node,Ray ray,float* rmin,float* rmax,Intersection* isect,double& mint,bool& hitted,int depth) const
	{			
		if (node != NULL)
		{
				if (node->box()->intersectp(ray,rmin,rmax))			
				{
					...
					if(node->is_leaf() && !node->is_empty()) {
						...
						{
							const Triangle* face = node->facelist()[j];
	...
	//for shadow rays
	void Kdtree::intersect_nodes(KdtreeNode* node,Ray ray,float* rmin,float* rmax,double& mint,bool& hitted,int depth) const {
	...
	bool Kdtree::examine_shadow(const Ray& r) const 
	{
		bool ishitted = false;						
		double mint = 1e30;
		float t0 = 0 ,t1 = 0;
	...
	bool Kdtree::examine(const Ray& r,Intersection* isect) const
	{					
		bool ishitted = false;						
		double mint = 1e30;
		float t0 = 0 ,t1 = 0;
	...
	void KdtreeNode::calculate_bounds(std::vector<Triangle*>& list) const
	{
		...
		m_bounds[0] = Point3(min_x,min_y,min_z);
		m_bounds[1] = Point3(max_x,max_y,max_z);
		...
		m_center = Vector3(midx,midy,midz);
		m_box = new BBox(m_bounds[0],m_bounds[1]);
	}
