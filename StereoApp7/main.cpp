...
void CameraHome(int mode)
{
    ...
    CROSSPROD(camera.vd, camera.vu, camera.r);

    Normalise(&camera.r);

    camera.r.x *= (camera.eyesep / 2.0);
    camera.r.y *= (camera.eyesep / 2.0);
    camera.r.z *= (camera.eyesep / 2.0);
}
...
void RotateCamera(int ix, int iy, int iz)
{
    ...
    dd = radius * radians;
    newvp.x = camera.vp.x + dd * ix * right.x + dd * iy * camera.vu.x - camera.pr.x;
    newvp.y = camera.vp.y + dd * ix * right.y + dd * iy * camera.vu.y - camera.pr.y;
    newvp.z = camera.vp.z + dd * ix * right.z + dd * iy * camera.vu.z - camera.pr.z;
...
//-------------------------------------------------------------------------------
/// \brief    Rotation of the Eye Seperation
///

void RotateEyeSeperation(XYZ p, int ix)
{
    XYZ u,q1,q2;

    double d, theta;

    theta=dtheta * PI / 180.0;
    
    q1.x = camera.vp.x + p.x - camera.pr.x;
    q1.y = camera.vp.y + p.y - camera.pr.y;
    q1.z = camera.vp.z + p.z - camera.pr.z;

    u.x = camera.vp.x - camera.pr.x;
    u.y = camera.vp.y - camera.pr.y;
    u.z = camera.vp.z - camera.pr.z;
    ...
    // Step 4
    q2.x = q1.x * cos(theta*ix) - q1.y * sin(theta*ix);
    q2.y = q1.x * sin(theta*ix) + q1.y * cos(theta*ix);
    q2.z = q1.z;
   ...
   // Inverse of step 1
   q1.x = q2.x + camera.pr.x;
   q1.y = q2.y + camera.pr.y;
   q1.z = q2.z + camera.pr.z;

   camera.r.x= q1.x - camera.vp.x;
   camera.r.y= q1.y - camera.vp.y;
   camera.r.z= q1.z - camera.vp.z;
}
...
void TranslateCamera(int ix, int iy)
{
    ...
    camera.vp.x += iy * camera.vu.x * delta;
    camera.vp.y += iy * camera.vu.y * delta;
    camera.vp.z += iy * camera.vu.z * delta;
    camera.pr.x += iy * camera.vu.x * delta;
    camera.pr.y += iy * camera.vu.y * delta;
    camera.pr.z += iy * camera.vu.z * delta;
...
void Display(void)
{
    double dist, ratio, radians, scale, wd2, ndfl,rotangle;
    ...
    if (stereo)
        ...
        rotangle = atan2(camera.vp.y , camera.vp.x);
        ...
        if (stereo)
        {
            ...
            for(int g=0; g<=1; g++)
            {
                ...
                if (g==0)
                {
                    glViewport(0, camera.screenheight/2, camera.screenwidth, camera.screenheight/2);

                    glScissor(0, camera.screenheight/2, camera.screenwidth, camera.screenheight/2);
                    ...
                    glFrustum(left, right, -0.2*bottom, 2.0*top, near, far);
                    ...
                    gluLookAt(camera.vp.x + camera.r.x, camera.vp.y + camera.r.y, camera.vp.z + camera.r.z, camera.vp.x + camera.r.x + camera.vd.x, camera.vp.y + camera.r.y + camera.vd.y, camera.vp.z + camera.r.z + camera.vd.z, camera.vu.x, camera.vu.y, camera.vu.z);
            ...
            // goruntunun diger taraftada gosterilmesi///////////////////////////////////////////
                ...
                if (g==1)
                {
                    glViewport(0, 0, camera.screenwidth, camera.screenheight/2);

                    glScissor(0, 0, camera.screenwidth, camera.screenheight/2);
                    ...
                    glFrustum(left, right, 2.0*bottom, 0.2*top, near, far);
...
void Keyboard(unsigned char key, int x, int y)
{
    switch (key)
    {
        ...
        case 'a':            // Rotate Eye SeperationP

        case 'A':
            RotateEyeSeperation(camera.r, 1);
            break;

        case 's':            // Rotate Eye Sepretion
...
int main(int argc,char** argv)
{
    ...
    if (fullscreen==TRUE)
    {
        glutGameModeString( "1280x1600:16@120" );//1280x800 1024x768 2048x768 2560x800 1280x1600
        glutEnterGameMode();
    }
